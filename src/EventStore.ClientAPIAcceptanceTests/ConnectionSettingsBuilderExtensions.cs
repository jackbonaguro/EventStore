		...
		public static ConnectionSettingsBuilder UseSsl(this ConnectionSettingsBuilder builder, SslType connType) {
			switch (connType) {
				...
				case SslType.WithAdminClientCertificate:
					...
					throw new NotImplementedException();
					#else
					using (var stream = typeof(EventStoreClientAPIFixture).Assembly.GetManifestResourceStream(typeof(EventStoreClientAPIFixture), "test_certificates.client_cn_admin.client_cn_admin.p12")) {
